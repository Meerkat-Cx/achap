				...
				<xsl:for-each select="evtsEstab">
						...
						<xsl:for-each select="descendant::controles">
							<xsl:for-each select="control">
								<xsl:apply-templates select="rdos">
									<xsl:with-param name="texto">
										Evento Control Establecimiento
										<xsl:value-of select="IDEvt"/>
									</xsl:with-param>
								</xsl:apply-templates>
								<xsl:for-each select="ordeniesAnimal/ordenieAnimal">
									<xsl:apply-templates select="rdos">
										<xsl:with-param name="texto">
											Evento Control Animal
											<xsl:value-of select="idOrdenie"/>
										</xsl:with-param>
									</xsl:apply-templates>
								</xsl:for-each>			
							</xsl:for-each>
						</xsl:for-each>
						<xsl:for-each select="descendant::bajasEvt">
							<xsl:for-each select="bajaEvt">
								<xsl:apply-templates select="rdos">
									<xsl:with-param name="texto">
										Evento Baja Evento
										<xsl:value-of select="IDEvt"/>
