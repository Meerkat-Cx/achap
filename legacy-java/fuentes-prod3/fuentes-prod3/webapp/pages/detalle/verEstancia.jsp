...
<html:html locale="true">
	...
	<body>
		<html:form action="/buscarEstancia.do" > 
	  <html:hidden name="busquedaEstanciaForm" property="idEst"/>
	...
	<table width="100%" class="bordeGris" cellSpacing="0" cellPadding="2" align="center" >
		...
		<tr> 
    	   	<td height="25" bgColor=#eff3e3 class=SubTitulo colspan="3">
        		<img height=15 hspace=2 src="pages/assets/images/flecha_titulos3a.gif" width=11 align=absMiddle>&nbsp;Datos del Establecimiento Sicel 3
            </td>
        </tr>
         <tr>
			<td class="celdaLabel">ID</td>
			<td class="celdaLabelSinAlign" colspan="2"  align="left">
						<c:out value="${busquedaEstanciaForm.idEst}"/>
			</td>
		</tr>
		...
		<tr>
			<td class="celdaLabel">
				Nombre: 
				 ...
				 <td class="celdaLabelSinAlign" colspan="2"  align="left">
									<c:out value="${busquedaEstanciaForm.nombre}"/>
			</td>			
		</tr>	
		...
		<tr>
			...
			<td class="celdaLabelSinAlign" colspan="2" align="left">
							<c:out value="${busquedaEstanciaForm.nombrePropietario}"/>
			</td>
		</tr>	
		<TR>
			<TD class="celdaLabel">
		...
		<tr>
			...
			<c:choose>
				<c:when test="${busquedaEstanciaForm.activo == 'false'}">
	...
	<table width="100%" class="bordeGris" cellSpacing="0" cellPadding="2" align="center">     						
		  <TD class=Titulo width="30%"><FONT color=#529b28>
	    		...
	    		<display:table name="tambos" align="left" width="40%" class="its" id="item" scope="request">        
		    	      <display:column align="centre" title="Id">			  	    
							<c:out  value="${item.id}"/>
			  	      </display:column>
			  	      <display:column align="centre" title="Nombre">
			    		    <c:out value="${item.nombreContacto}"/>
			  	      </display:column>
					   <display:column align="centre" title="Id Establecimiento">
			    		    <c:out value="${item.estancia.id}"/>
			  	      </display:column>  
					  <display:column align="centre" title="Id Propietario">
			    		    <c:out value="${item.propietario.id}"/>
			  	      </display:column>
