...
<html:form  action="/informeProcesosAction"  enctype="multipart/form-data" onsubmit="return validate();">
	...
	<input type="hidden" name="unaRegional" value="<c:out value="${unEst}"/>" />
			...
			<table width="100%" class="bordeGris" cellSpacing="0" cellPadding="3"
				...
				<tr>
					...
					<td class="celdaLabelSinAlign" colspan="2"  >
						<c:if test="${unaReg == 'si'}">
							<input type="radio" name="checkFiltro10" onclick="cambiarFiltro10()" checked>una Regional	
						</c:if>
						<c:if test="${unaReg == 'no'}">
							<input type="radio" name="checkFiltro10" onclick="cambiarFiltro10()">una Regional
						</c:if>
					</td>
					<td class="celdaLabelSinAlign" colspan="1" align="left">
						<html:select onchange="changeComboRegional2()" property="regionalIdCombo2" styleId="comboRegional2"  name="reportesEcloForm" styleClass="formfields" style="width:400px">
						<html:options collection="regionales" property="id" labelProperty="idNombre" />
						</html:select>
					</td>
				<tr>
				<tr>
					...
					<td class="celdaLabelSinAlign" colspan="2">
						<c:if test="${unaReg == 'si'}">
							<input type="radio" name="checkFiltro10" onclick="cambiarFiltro10()">todas las regionales
						</c:if>
						<c:if test="${unaReg == 'no'}">
							<input type="radio" name="checkFiltro10" onclick="cambiarFiltro10()" checked>todas las regionales
						</c:if>
					</td>
				</tr>
			</table>
		...
		<tr>
			...
			<table width="100%" class="bordeGris" cellSpacing="0" cellPadding="2">
				...
				<tr>
					<td class="celdaLabel"></td>
					<td class="celdaLabelSinAlign" colspan="2">
					<c:if test="${unEstTodos == 'si'}">
						<input type="radio" name="checkFiltro4A"
							onclick="cambiarFiltro4A()" checked>un tambo
					</c:if> 
					<c:if test="${unEstTodos == 'no'}">
						<input type="radio" name="checkFiltro4A"
							onclick="cambiarFiltro4A()">un tambo	
					</c:if>
				</tr>
				<tr>
					<td class="celdaLabel"></strong></td>
   ...
   function initBuscarEstablecimientosPropios(){
    ...
    if (document.forms[0]["tipoLogin"].value=="ADMINISTRADOR") {
		cambiarFiltro1();
	}
	initializeMenus();
